...
- `content-recomposer.ts`
  - 정적 HTML body 트리 파싱/저장 + text/image/video/iframe 블록 재구성

## 설계 포인트

- extractor는 "결정"이 아닌 "후보 수집/정규화"에 집중
- 정적/동적 본문은 같은 블록 렌더 규칙으로 처리
- recomposer는 `parse|capture(body tree) → sanitize → store tree → block render → quality score → renderDocument(index.html+css)` 파이프라인
